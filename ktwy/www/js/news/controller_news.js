angular.module('ktwy.controllers')
    ...
    $scope.open_wnd_news_wy_edit = function (iid) {
      ...
      else
      {
        $scope.news_wy.getmodel(iid).then(function(rtn){
          ...
          console.info(rtn);
          $scope.news_wy.model.md_url = wwwurl + rtn.md_url.substr(1);
          $scope.news_wy.model.bg_url = wwwurl + rtn.bg_url.substr(1);
        },function(rtn){
  ...
  .controller('news_wy_edit', function ($scope, $stateParams, $state, $log, $ionicPopup, $ionicModal, $ionicActionSheet,$ionicHistory, service_roomselect, service_usercenter, service_dict, NativePlugin, service_wy_resource, service_news_wy) {
    ...
    $scope.savenews_wy=function()
    {
      ...
      if($scope.news_wy.model.status!="0" )
      {
        $ionicPopup.alert({
          title: '提醒',
          okType:'button-orange',
    ...
    $scope.receivenews_wy=function()
    {
      ...
      confirmPopup.then(function(res) {
        if(res) {
          ...
          //savemodel
          $scope.news_wy.model.receiver=$scope.usercenter.name;
          $scope.news_wy.model.receive_dt=util.today('-');
          $scope.news_wy.model.status='1';
          //savemodel
          $scope.news_wy.savemodel().then(function (rtn) {
            //保存完成
            $ionicPopup.alert({
    ...
    $scope.openSelectRoomWnd = function () {
      ...
      if($scope.news_wy.model.status=='1')return;
    ...
    $scope.closeSelectRoomWnd = function (roomid, roompath) {
      ...
      $scope.news_wy.model.roomid = roomid;
      $scope.news_wy.model.roompath = roompath;
    ...
    $scope.getPicture = function (ptype) {
      ...
      NativePlugin.GetPicture(function (imageData) {
        ...
        NativePlugin.FileTrans(NativePlugin.PictureModel.image_url, function (rtn) {
          ...
          if(ptype=="md") {
            $scope.news_wy.model.md_url = wwwurl + rtn.substr(1);
          }
          else
          {
            $scope.news_wy.model.bg_url = wwwurl + rtn.substr(1);
          }
          service_wy_resource.SaveResource(function (rtn) {
            if(ptype=="md") {
              $scope.news_wy.model.md = jsondal.AnaRtn(rtn);
            }
            else
            {
              $scope.news_wy.model.bg = jsondal.AnaRtn(rtn);
            }
            $scope.$apply();
          }, function (rtn) {
          });
    ...
    $scope.add_viewPhoto = function (ptype) {
      ...
      if(ptype=="md")
      {
        url=$scope.news_wy.model.md_url;
      }
      else
      {
        url=$scope.news_wy.model.bg_url;
      }
