...
public class OpenUDID_manager implements ServiceConnection
{
	...
	public void onServiceConnected(ComponentName className, IBinder service)
	{
		...
		try
		{
			...
			if (data.readInt() == reply.readInt()) // Check if the service returns us this number
			{
				...
				{ // if valid OpenUDID, save it
					if (LOG)
						LogHelper.d(TAG, "Received " + _openUDID);
		...
		{
			if (LOG)
				LogHelper.e(TAG, "RemoteException: " + e.getMessage());
		}
	...
	{
		if (LOG)
			LogHelper.d(TAG, "Generating openUDID");
	...
	private void startService()
	{
		...
		{ // There are some Intents untested
			if (LOG)
				LogHelper.d(TAG, "Trying service " + mMatchingIntents.get(0).loadLabel(mContext.getPackageManager()));
		...
		{ // No more service to test
			...
			if (OpenUDID == null) // No OpenUDID was chosen, generate one
				generateOpenUDID();
			if (LOG)
				LogHelper.d(TAG, "OpenUDID: " + OpenUDID);
	...
	{
		if (!mInitialized)
			LogHelper.e("OpenUDID", "Initialisation isn't done");
		return OpenUDID;
	}
	...
	public static void sync(Context context)
	{
		...
		if (OpenUDID == null) // Not found
		{
			...
			if (LOG)
				LogHelper.d(TAG, manager.mMatchingIntents.size() + " services matches OpenUDID");
